...
def default_probs_dict():
    return {'B': 0, 'M': 0, 'E': 0, 'S': 0, 'O': 0, '': 0, 'BACK_OFF': 0}
...
prob_dict['O'] = default_TnT_probs_dict()
...
N = sum([prob_dict['BACK_OFF']['BACK_OFF'][s.value] for s in NameStatus])
for s1 in NameStatus:
    for s2 in NameStatus:
        for s3 in NameStatus:
            sv1 = s1.value
            sv2 = s2.value
            sv3 = s3.value
...
for key1, sub_dict in prob_dict.items():
    if key == '_C_':
        continue
    if key1 not in "BMESO":
        continue
    for key2, value in sub_dict.items():
        if key2 not in "BMESO":
            continue
        total = 0
...
for s in NameStatus:
    ...
    for key, value in prob_dict['_C_'][s.value].items():
        if key not in 'BMESO' and key != 'BACK_OFF':
            total += value
